...
// PATCHED, and not by mitex. See ../../../PATCHES.md.
//
// The three handlers below took the contents of \hspace{}, \vspace{} and \raisebox{} and passed
// them straight to `eval`, which runs them as Typst code. That content is whatever the author typed
// between the braces, so a plain markdown file holding \hspace{range(999999999).len()*1pt} is a
// program, and the compiler runs inside the editor's own process rather than in a sandbox beside
// it: the measured result was nine gigabytes resident and a dead app, taking any unsaved buffer
// with it. Nothing else in the file evaluates document text.
//
// A length is all any of the three can use, so a length is all this accepts. Anything else is zero
// rather than an error, because a document that will not export is a worse answer than a document
// whose spacing command was ignored, and the author can see the second one on the page.
#let mitex-safe-length(raw) = {
  if type(raw) != str {
    0pt
  } else {
    let text = raw.trim()
    if text.match(regex("^[+-]?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)(pt|mm|cm|in|em)$")) != none {
      eval(text)
    } else {
      0pt
    }
...
#let (
  ...
  raisebox: define-cmd(2, handle: (sp, it) => text(baseline: -mitex-safe-length(get-tex-str(sp)), it)),
  ...
  vspace: define-cmd(1, handle: it => v(mitex-safe-length(get-tex-str(it)))),
  hspace: define-cmd(1, handle: it => h(mitex-safe-length(get-tex-str(it)))),
